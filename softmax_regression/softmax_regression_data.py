...
class SoftmaxRegressionData():
    def __init__(self, data_path):
        self.NUMPY_DATA = np.genfromtxt(data_path, delimiter = ',', skip_header=1)

        self.X_NUMPY, self.Y_NUMPY = self.NUMPY_DATA[:, :-1], self.NUMPY_DATA[:, -1][:, np.newaxis].astype(int)
        ...
        self.Y_TENSOR_TRAIN = torch.LongTensor(self.Y_NUMPY_TRAIN).flatten()
        self.Y_TENSOR_TEST = torch.LongTensor(self.Y_NUMPY_TEST).flatten()

        self.LEARN_RATE = 0.0005
        self.NUMBER_OF_EPOCHS = 1000
        self.NUMBER_OF_CLASSES = np.unique(self.Y_NUMPY).shape[0]
    ...
    def get_parameters(self):
        return self.LEARN_RATE, self.NUMBER_OF_EPOCHS, self.NUMBER_OF_CLASSES
