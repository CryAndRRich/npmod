...
def expand_cluster(features: np.ndarray,
    ...
    """
    neighbors = region_query(features, point_idx, eps)
    if len(neighbors) < min_samples:
        labels[point_idx] = -1  # noise
        return False

    labels[point_idx] = cluster_id
    seeds = [n for n in neighbors if n != point_idx]
    ...
    while seeds:
        ...
        if labels[current] == 0:  # unvisited
            labels[current] = cluster_id
            current_neighbors = region_query(features, current, eps)
...
class DBSCAN():
    ...
    def fit(self, features: np.ndarray) -> np.ndarray:
        ...
        for idx in range(n_samples):
            if labels[idx] != 0:  # already visited
                continue
    ...
    def __str__(self) -> str:
        return "DBSCAN (Density-Based Spatial Clustering of Applications)"
