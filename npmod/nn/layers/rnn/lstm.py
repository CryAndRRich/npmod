...
class LSTMCell(Layer):
    ...
    def forward(self, x: np.ndarray, h_prev: np.ndarray, c_prev: np.ndarray):
        ...
        # Forget gate
        f = Sigmoid(concat.dot(self.W_f.T) + self.b_f)
        # Input gate
        i = Sigmoid(concat.dot(self.W_i.T) + self.b_i)
        # Cell gate
        c_hat = np.tanh(concat.dot(self.W_c.T) + self.b_c)
        # Output gate
        o = Sigmoid(concat.dot(self.W_o.T) + self.b_o)
    ...
    def backward(self, 
        ...
        d_concat = (do_input.dot(self.W_o)) + (di_input.dot(self.W_i)) + (df_input.dot(self.W_f)) + (dc_hat_input.dot(self.W_c))

        # Split d_concat to obtain dh_prev and dx gradient
...
class LSTM(Layer):
    ...
    def forward(self, 
        ...
        for t in range(seq_len):
            ...
            self.cache.append((h_prev, c_prev, x[:, t, :], h_next, c_next))
    ...
    def backward(self, 
        ...
        for t in reversed(range(seq_len)):
            # Sum gradients from output and next states
